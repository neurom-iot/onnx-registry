from flask import Flask, render_template, request, redirect,send_file , send_from_directory, redirect
import os, time
    ...
    if request.method == 'POST':
        ...
        for x in files:
            if x == request.form['file']: # 파일이름이면
                netron.start(file=x, browse=False, port=netron_port, host=server)
...
if __name__ == '__main__':
    app.run(host=server, port=flask_port, debug = True)
